# Strange Theme — 项目核心规范

## 项目概述

Strange Theme 是一个 dark 主题扩展，专为 Vue SFC 开发优化，支持 Trae、VS Code、Cursor。通过三区块分层着色和语义高亮，让 Vue 模板中的逻辑代码与普通 HTML 在视觉上强烈区分。
...
└── themes/
    └── strange-theme.json        # 主题定义（自包含，无 include）
```
...
| 前缀 | 场景 | 示例 |
|------|------|------|
| `feat:` | 新功能/特性 | `feat: 添加 Cursor 编辑器兼容` |
| `fix:` | 修 bug | `fix: 修复函数颜色与 script 标签冲突` |
| `refactor:` | 重构 | `refactor: 主题重命名为 Strange Theme` |
| `chore:` | 杂项（配置、工具链） | `chore: 更新 gitignore 规则` |
